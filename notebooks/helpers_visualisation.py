...
def plot_path(graph_nodes, graph_strong_connected, route=[]):
    ...
    accordion = widgets.Accordion(children=[textbox_src,textbox_dst,textbox_time])
    ...
    #widget to display trip duration     
    duration_widget =  widgets.HTML(
    value="",
    description=""
)
  ...
  # adding stations marker
        ...
        if validate():
            with fig.batch_update() :
                ...
                try : 
                    ...
                    # compute the journey
                    (hour,minute,second,found_journey) =  final_journey(graph,graph_nodes,textbox_src.value,textbox_dst.value,string_datetime(textbox_time.value))
                    print(found_journey)
                    ...
                    if(found_journey.shape[0]> 0):
                       ...
                       # update duration
                        ...
                        duration_widget.value = "<font size='1'><b>Duration</b></font>: <font color='black'><b> Your trip is {hours} hours, {minutes} minutes and {seconds} seconds long</b></font>"\
                                                            .format(hours =hour, minutes=minute, seconds=second)
                        # update figure value
                    ...
                    duration_widget.value = "<font color='red'><b>TERMINATED</b> (no stations found for your arrival time)</font>"
                    found_journey = pd.DataFrame()
    # embed outer widgets
    accordion_box = widgets.HBox([button_apply,computation_state,duration_widget])
    ...
    tab.children  =  [widgets.VBox([accordion,accordion_box, fig]),route_plan,duration_widget]
    return tab
